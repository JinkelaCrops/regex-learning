...
我们使用的文本是医疗领域的50000句中英双语句对，中间以空格分隔，有以下的整理任务。
* 将中英文句对拆分

如果中英文中间没有空格。
* 将中英文句对拆分，拆分规则是英文中没有中文
* 将中英文句对拆分，拆分规则是中文中没有英文

拆分以后的中英文句对以`" ||| "`分隔。想象一下我们有以下的检索任务：
* 数字，包括整数和小数
* 英文缩写，骆驼拼写等特殊单词
* 带有连字符的英文单词，包括化合物名字
...
2. 回溯
    ...
    使用`a@ab`和字符串`aaaaab`匹配，首先侵吞全部的`a`，然后吐出一位，最后匹配成功
...
4. 叠词
    ...
    `\b(?<w1>[A-Za-z]+)\k<w1>(?<w2>[A-Za-z]+)\k<w2>\b`
